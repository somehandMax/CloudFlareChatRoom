// =============================================================================
// Cloudflare Worker 加密聊天室 - 第三阶段（Durable Object + 端到端加密 + 文件透传）
// =============================================================================
// 架构（重要）：
//   - 每个房间口令映射到唯一的 Durable Object 实例（ChatRoom）。
//     同一房间的所有 WebSocket 连接都进入同一个 DO 实例，连接表/广播/历史
//     都在该实例内存中，因此用户列表、消息、文件信令能可靠地实时互通。
//     （普通 Worker 的全局变量在多 isolate 下不共享，会导致广播失效。）
//
// 安全模型：
//   - 端到端加密：AES 密钥由【前端】用 PBKDF2(密码/口令 + 房间盐) 派生，
//     服务器永远不接触明文密钥，也不生成/存储/下发密钥。
//   - 密码认证：前端先算 SHA-256(盐+密码)=authHash，服务器再存 SHA-256(盐+authHash)。
//   - 防中间人：前端强制 WSS、CryptoJS 走 SRI、响应头加 HSTS/CSP 等。
//   - 文件传输：前端用房间密钥 AES 加密分片，DO 仅定向透传，绝不解密/存储。
//
// 房间标识：6 位纯数字口令。KV 绑定名：ROOM_KV。DO 绑定名：CHAT_ROOM。
// =============================================================================
